...
describe("Command-Line Scripts", () => {
  fs.readdirSync(path.resolve(rootDir, 'test', 'integration', 'data', 'source')).forEach(example => {
    describe(example, () => {
      const sourcePath = path.resolve(dataDir, 'source', example)
      const redactedPath = path.resolve(dataDir, 'redacted', example)
      const translatedPath = path.resolve(dataDir, 'translated', example)
      const restoredPath = path.resolve(dataDir, 'restored', example)
