...
const { tiplink } = require("@code-dot-org/remark-plugins");
  ...
  describe("redact", () => {
    it("redacts tiplinks", () => {
      ...
      expect(output).toEqual(
        "This is some text with an inline labeled tip: [][0]\n"
      );
    });
    ...
    it("redacts basic discussion links", () => {
      const input =
        "This is some text with an inline labeled tip: " +
        basicDiscussionMarkdown;
      const output = processor.sourceToRedacted(input);
      expect(output).toEqual(
        "This is some text with an inline labeled tip: [][0]\n"
      );
